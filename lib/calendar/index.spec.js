...
describe('Calendar', () => {
  ...
  describe('Month days', () => {
    ...
    it('should not include extra days with `hideExtraDays={true}` prop', () => {
      const drv = new CalendarDriver().withDefaultProps({
        hideExtraDays: true
      }).render();
    ...
    it('should gray out dates not in interval between `minDate` and `maxDate`', () => {
      ...
      }).render(); // Disabled dates

      expect(drv.getDay('2020-04-09').getTextStyle()).toEqual(partial({
        color: textDisabledColor
      }));
      expect(drv.getDay('2020-04-12').getTextStyle()).toEqual(partial({
        color: textDisabledColor
      })); // Enabled dates

      expect(drv.getDay('2020-04-10').getTextStyle()).not.toEqual(partial({
        color: textDisabledColor
      }));
      expect(drv.getDay('2020-04-11').getTextStyle()).not.toEqual(partial({
        color: textDisabledColor
      }));
    });
    it('should disable touch events for disabled dates with `disableAllTouchEventsForDisabledDays`', () => {
      ...
      new CalendarDriver().withDefaultProps({
        onDayPress,
        markedDates: {
          [date]: {
            disabled: true
    ...
    describe('Basic Day', () => {
      ...
      it('should mark selected day', () => {
        ...
        expect(drv.getDay(date).getStyle()).toEqual(partial({
          backgroundColor: selectedColor,
          borderRadius: 16
        }));
        expect(drv.getDay(date).getTextStyle()).toEqual(partial({
    ...
    describe('Accessibility labels', () => {
      ...
      it('should have correct label for selected date with no markings', () => {
        ...
        expect(drv.getDay('2020-04-10').getAccessibilityLabel()).toBe('Friday 10 April 2020 selected You have no entries for this day');
      });
      it('should have correct label for selected date with markings', () => {
        ...
        expect(drv.getDay('2020-04-10').getAccessibilityLabel()).toBe('Friday 10 April 2020 selected You have entries for this day');
      });
  ...
  describe('Header', () => {
    ...
    it('should render custom header via `renderHeader` prop', () => {
      ...
      const renderHeader = jest.fn().mockReturnValue( /*#__PURE__*/React.createElement('Text', {}, text));
      const drv = new CalendarDriver().withDefaultProps({
        renderHeader
      }).render();
    ...
    it('should render custom header component via `customHeader` prop', () => {
      ...
      const customHeader = props => /*#__PURE__*/React.createElement('Text', props, text);
    ...
    it('should have loading indicator with `displayLoadingIndicator` prop when `markedDates` collection does not have a value for every day of the month', () => {
      expect(new CalendarDriver().withDefaultProps({
        current: '2020-04-01',
        displayLoadingIndicator: true
      }).render().getHeader().getLoadingIndicator()).toBeDefined();
    });
    it('should not have loading indicator with `displayLoadingIndicator` prop when `markedDates` collection has a value for every day of the month', () => {
      ...
      expect(new CalendarDriver().withDefaultProps({
        ...
        displayLoadingIndicator: true,
        markedDates
      }).render().getHeader().getLoadingIndicator()).toBeUndefined();
    });
    describe('Week days', () => {
    ...
    describe('Arrows', () => {
      ...
      it('should render custom arrows using `renderArrow` prop', () => {
        const renderArrow = jest.fn().mockImplementation(direction => /*#__PURE__*/React.createElement('Text', {}, direction));
        const drv = new CalendarDriver().withDefaultProps({
          renderArrow
        }).render();
